...
namespace EDennis.AspNetCore.Utils.Middleware {
    ...
    /// </summary>
    public abstract class ResponseMiddleware {
        ...
        protected abstract void Setup(HttpContext context);

        /// <summary>
        /// Implement this method in order to process the
        ...
        protected abstract void Process(Stream inStream, Stream outStream);

        /// <summary>
        /// Implement this method in order to perform some
        /// teardown operations (e.g., close a connection) 
        /// after processing the response body.
        /// </summary>
        ...
        public async Task Invoke(HttpContext context) {
            ...
            using (var delegatedBodyStream = new MemoryStream()) {
                ...
                using (var processOutputStream = new MemoryStream()) {

                    //rewind stream
                    delegatedBodyStream.Seek(0, SeekOrigin.Begin);

                    //asynchronously setup and invoke the transformation
                    await Task.Run(() => { Setup(context); });
                    await Task.Run(() => { Process(delegatedBodyStream, processOutputStream); });
                    await Task.Run(() => { Teardown(); });

                    //rewind stream
                    processOutputStream.Seek(0, SeekOrigin.Begin);

                    //write to the official body stream, which is the
                    //only way to return a response
                    await processOutputStream.CopyToAsync(officialBodyStream);
                }
            }
        }
